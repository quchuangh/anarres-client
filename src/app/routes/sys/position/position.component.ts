...
import { SysPositionCreateComponent } from './modal/create.component';
import { SysPositionEditComponent } from './modal/edit.component';
import { SysPositionViewComponent } from './modal/view.component';
...
export class SysPositionComponent implements OnInit {
  ...
  schema: SFSchema = asFilterInputPropertiesUI({
    properties: {
      positionCode: { type: 'string', title: '职位编号', ui: { options: ['contains'] } },
      name: { type: 'string', title: '名称', ui: { options: ['contains'] } },
      organizationCode: { type: 'string', title: '组织编号', ui: { options: ['contains'] } },
      enabled: {
        type: 'array',
        title: '是否启用',
        ui: {
      ...
      roleCode: { type: 'string', title: '自带角色', ui: { options: ['contains'] } },
  ...
  columnDefs: AclColDef[] = [
    ...
    { headerName: '职位编号', field: 'positionCode' },
    { headerName: '名称', field: 'name' },
    { headerName: '简介', field: 'description' },
    { headerName: '组织编号', field: 'organizationCode' },
    { headerName: '是否启用', field: 'enabled' },
    { headerName: '自带角色', field: 'roleCode' },
  ...
  constructor(private http: _HttpClient, private aclService: ACLService, private message: NzMessageService, private modal: NzModalService) {
    ...
    this.dataSource = DataSourceUtils.rowQuery(http, '/api/position/query', (r) => r);
  }
  ...
  onGridReady(e: { event: GridReadyEvent; gridTable: NgxGridTableComponent }): void {
    ...
    this.table.addMenu({
      name: '删除',
      show: 'selected',
      acl: { ability: ['position:delete'] },
      callback: (selected) => {
        this.http.delete(`/api/position/delete/${this.idGetter(selected)}`).subscribe((value) => {
  ...
  onCreate(): void {
    this.modal
      .create({
        nzContent: SysPositionCreateComponent,
        nzFooter: null,
  ...
  onEdit(cell: ICellRendererParams, row: RowNode): void {
    this.modal
      .create({
        nzContent: SysPositionEditComponent,
  ...
  onView(cell: ICellRendererParams, row: RowNode): void {
    this.modal.create({
      nzContent: SysPositionViewComponent,
      nzComponentParams: { record: row.data },
      nzFooter: null,
      nzMaskClosable: true,
    });
